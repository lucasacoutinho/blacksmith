...
const statusColor = (status: DiffEntry['status']): string => {
  switch (status) {
    case 'improved': return 'var(--vscode-charts-green, #4ec9b0)';
    case 'regressed': return 'var(--vscode-charts-red, #f44747)';
    case 'added': return 'var(--vscode-charts-yellow, #cca700)';
    case 'removed': return 'var(--vscode-charts-purple, #b180d7)';
    case 'unchanged': return 'var(--vscode-foreground)';
  }
};
...
const DiffSummary = memo(function DiffSummary({ diff }: { diff: DiffResult }) {
  ...
  const deltaColor = diff.totalDelta > 0
    ...
    <div className="diff-summary">
      <span className="diff-summary-text">
        ...
        <span style={{ color: deltaColor }}>
          ({formatDeltaPct(diff.totalDeltaPct)})
        </span>
        {' | '}
...
export const DiffProfile = memo(function DiffProfile() {
  ...
  const sorted = [...filtered].sort((a, b) => {
    ...
    switch (sortKey) {
      ...
      case 'file': cmp = a.file.localeCompare(b.file); break;
      case 'selfCost': cmp = a.selfDelta - b.selfDelta; break;
      case 'totalCost': cmp = a.totalDelta - b.totalDelta; break;
      case 'calls': cmp = (a.callsB - a.callsA) - (b.callsB - b.callsA); break;
      default: cmp = a.totalDelta - b.totalDelta; break;
    }
  ...
  return (
    <div className="table-container" ref={containerRef}>
      ...
      <div className="virtual-header diff-header">
        <div className={`diff-cell ${sortClass('name')}`} onClick={() => setSort('name')}>Function</div>
        <div className={`diff-cell ${sortClass('file')}`} onClick={() => setSort('file')}>File</div>
        <div className="diff-cell diff-num">Self A</div>
        <div className="diff-cell diff-num">Self B</div>
        <div className={`diff-cell diff-num ${sortClass('selfCost')}`} onClick={() => setSort('selfCost')}>Self Δ</div>
        <div className="diff-cell diff-num">Total A</div>
        <div className="diff-cell diff-num">Total B</div>
        <div className={`diff-cell diff-num ${sortClass('totalCost')}`} onClick={() => setSort('totalCost')}>Total Δ</div>
        <div className="diff-cell diff-status">Status</div>
      </div>
      <List height={height || 400} itemCount={sorted.length} itemSize={LAYOUT.ROW_HEIGHT} width="100%">
        {({ index, style }) => (
          <DiffRow
            index={index}
            style={style}
